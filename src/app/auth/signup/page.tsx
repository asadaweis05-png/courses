...
import { Mail, Lock, User, ArrowRight, Eye, EyeOff, Sparkles, BookOpen, Users, Award } from "lucide-react";
  ...
  return (
    <div className="pt-16 min-h-screen flex">
      ...
      <div className="hidden lg:flex flex-col justify-center items-center w-1/2 relative overflow-hidden px-12"
        ...
        <div className="absolute top-1/3 right-1/4 w-80 h-80 rounded-full blur-[100px] pointer-events-none"
          style={{ background: "radial-gradient(circle, rgba(124,58,237,0.12), transparent 70%)" }} />
        <div className="absolute bottom-1/3 left-1/3 w-60 h-60 rounded-full blur-[80px] pointer-events-none"
        ...
        <div className="relative z-10 max-w-md">
          ...
          <p className="text-[var(--text-muted)] text-sm leading-relaxed mb-10">
            Create your free account and join thousands of learners mastering AI, Technology, and Business.
          </p>
          <div className="space-y-4">
            {[
              { icon: BookOpen, text: "Access All Courses Free", color: "stat-icon-cyan" },
              { icon: Users, text: "Learn in Somali or English", color: "stat-icon-purple" },
              { icon: Award, text: "Track Progress & Get Certified", color: "stat-icon-green" },
            ].map((item, i) => (
        ...
        <div className="w-full max-w-md">
          <div className="glass-card p-8 md:p-10 animate-up" style={{ cursor: "default" }}>
            <div className="text-center mb-8">
              ...
              <p className="text-[var(--text-muted)] text-sm mt-2">Ku soo biir — Join for free</p>
            </div>

            <form onSubmit={handleSignup} className="space-y-5">
              <div>
                <label className="text-xs font-semibold text-[var(--text-secondary)] mb-2 block">Full Name</label>
                    ...
                    <div className="flex-1 flex gap-1">
                      {[1, 2, 3].map(level => (
                        <div key={level} className="flex-1 h-1 rounded-full transition-all duration-300"
                          style={{ background: passStrength >= level ? strengthColors[passStrength] : "rgba(255,255,255,0.06)" }} />
                      ))}
                    </div>
                    <span className="text-[10px] font-bold" style={{ color: strengthColors[passStrength] }}>
              ...
              <button type="submit" disabled={loading}
                ...
                {loading ? <><div className="spinner" /> Creating account...</> : <>Sign Up Free <ArrowRight size={16} /></>}
              </button>
            </form>
            ...
            <p className="text-center text-sm text-[var(--text-muted)]">
              ...
              <Link href="/auth/login" className="text-[var(--accent)] font-semibold no-underline hover:underline">
                Login
              </Link>
            </p>
          </div>
